...
namespace Colorize {
  ...
  int cmvaddch(color_t color, unsigned attrs, int y, int x, const int ch);
  ...
  int cmvaddch(color_t color, int y, int x, const int ch);

#ifdef TYPOS_DEBUG
  constexpr const char * clrtostr(color_t color) {
    constexpr const char * strs[COLORIZE_MAX + 1] = {"COLORIZE_DEFAULT",
                                                     "COLORIZE_OK",
                                                     "COLORIZE_WARN",
                                                     "COLORIZE_ERROR",
                                                     "COLORIZE_INFO",
    ...
    return strs[COLORIZE_MIN > color || COLORIZE_MAX < color ? COLORIZE_MAX : color];
  }
#endif
}; // namespace Colorize
